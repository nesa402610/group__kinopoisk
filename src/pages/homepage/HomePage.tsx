...
export function Homepage() {
  ...
  return (
    <div className="w-full container flex flex-col justify-center gap-5 py-3">
      ...
      {(bestFilmsFetching || !bestFilmsSuccess) ? <Loader /> : <FilmsCarousel films={bestFilms.films!} page='home' />}

      <h1 className="text-3xl text-center">Популярные фильмы</h1>
      {(popularFilmsFetching || !popularFilmsSuccess) ? <Loader /> : <FilmsCarousel films={popularFilms.films!} page='home' />}
      <h1 className="text-3xl text-center">Ожидаемые Фильмы</h1>
      {(awaitFilmsFetching || !awaitFilmsSuccess) ? <Loader /> : <FilmsCarousel films={awaitFilms.films!} page='home' />}
    </div>
  )
}
